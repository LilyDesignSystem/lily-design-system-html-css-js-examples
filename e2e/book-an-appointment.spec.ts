...
// Plan P6-T3: port of the "book an appointment" flagship composed-page
// pattern (plan P6-T2, canonical in the SvelteKit examples app) to this
// plain-HTML + vanilla-JS app. Full write-up:
// docs/patterns/book-an-appointment.md.
//
// This spec walks the entire flow end to end -- happy path, per-step
// validation, the "Change" links back from check-your-answers, the
// Back button, and the "book another appointment" reset -- then scans
// every distinct state with axe. Served at /book-an-appointment.html
// (this app's flat-.html routing convention; see e2e/accessibility.spec.ts).
...
test.describe('book an appointment: happy path', () => {
  test('reason -> date and time -> your details -> check answers -> confirmation', async ({ page }) => {
    await page.goto('/book-an-appointment.html');
