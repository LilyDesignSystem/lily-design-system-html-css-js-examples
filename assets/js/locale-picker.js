...
var RTL_LANGUAGE_TAGS = /* @__PURE__ */ new Set([
  "ar",
  // Arabic
  "arc",
  // Aramaic
  ...
  // Yiddish (legacy)
  "ks",
  // Kashmiri (commonly written in Arabic script)
  "ku",
  // Kurdish (umbrella; commonly Arabic script)
  "mzn",
  // Mazanderani
  "ps",
  // Pashto
...
var RTL_SCRIPT_SUBTAGS = /* @__PURE__ */ new Set([
  "arab",
  // Arabic
  "hebr",
  // Hebrew
  ...
  // N'Ko
  "mong",
  // Mongolian (traditional, written top-to-bottom but RTL line flow)
  "adlm"
  // Adlam
]);
...
function matchNavigatorLanguage(navLangs, locales) {
...
var LocalePicker = class extends HTMLElement {
  ...
  #resolveInitialValue() {
    ...
    if (!initial && this.detectFromNavigator && typeof navigator !== "undefined") {
      const navLangs = navigator.languages && navigator.languages.length > 0 ? Array.from(navigator.languages) : navigator.language ? [navigator.language] : [];
      initial = matchNavigatorLanguage(navLangs, this.#locales);
    }
    if (!initial) {
      initial = this.defaultValue || (this.#locales.includes("en") ? "en" : this.#locales[0]) || "";
    }
